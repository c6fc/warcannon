...
local iam_role(name, description, policy_attachments, inline_policies, trust_policy, instance_profile = false) = std.prune({
	aws_iam_role: {
		[name]: {
			name: name,
	...
	aws_iam_instance_profile: if instance_profile then {
		[name]: {
			name: name,
