{
	...
	base_vpc(basename, region, cidr, azs, segmentoffset, baseendpoints):: {
		...
		resource: {
			...
			aws_security_group_rule: {
				["%s-default-ingress" % name]: vpc.sg_single("any:self", "%s-default" % name, "ingress"),
				["%s-default-egress" % name]: vpc.sg_single("all", "%s-default" % name, "egress"),
			}
		} + std.prune({
			aws_vpc_endpoint: {
				["%s-%s" % [name, endpoint[0]]]: {
	...
	sg_single(shorthand, sg = null, type = null, source = '')::
	...
	private_vpc(basename, region, cidr, azs, segmentoffset, baseendpoints):: $.base_vpc(basename, region, cidr, azs, segmentoffset, baseendpoints) + {
		// Overload the name for scalability.
		local name = "%s-%s" % [basename, region],
		local provider = "aws.%s" % [region],
		local vpc_id = "${aws_vpc.%s.id}" % [name],
