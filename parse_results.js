...
glob(results_path + '/*_of_*.json', function(err, files) {
	...
	files.forEach((f) => {
		...
		Object.keys(contents.regex_hits).forEach((r) => {
			if (!metrics.regex_hits.hasOwnProperty(r)) {
				metrics.regex_hits[r] = { domains: {}, hits: 0 };
			}
			...
			Object.keys(contents.regex_hits[r].domains).forEach((d) => {
				...
				*/
				...
				if (!metrics.regex_hits[r].domains.hasOwnProperty(d)) {
					metrics.regex_hits[r].domains[d] = [];
				}
